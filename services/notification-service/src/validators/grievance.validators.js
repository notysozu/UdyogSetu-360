...
function validateGrievanceCreate(input = {}) {
  ...
  if (input.priority && !['low', 'normal', 'high', 'urgent'].includes(input.priority)) errors.push('priority is invalid');
  if (input.departmentCode && !DEPARTMENT_CODE_VALUES.includes(input.departmentCode)) errors.push('departmentCode is invalid');
  return errors;
}
...
function validateStatusChange(input = {}) {
  ...
  if (['rejected', 'resolved', 'closed', 'reopened'].includes(input.nextStatus) && !input.reason) errors.push('reason is required');
